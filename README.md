...
- 订阅直连失败时自动复用 OpenClash 本机代理与现有认证重试，无需额外配置
- 后台操作只在页头显示简短状态，不使用持续占据页面的进度条
- 原子写回配置文件，并在写回前自动备份
- 自动重启 OpenClash
...
工具会自动生成并维护 OpenClash 本地配置。订阅卡片的“刷新”更新该订阅；当前订阅刷新后自动过滤并应用，待机订阅只更新私有缓存。地区开关和待用节点先形成页面草稿，点击一次“保存并应用”后，工具直接使用最后一次成功下载的本地 YAML 完成一次过滤、必要的重载、节点选择和验证，不会为每个开关分别请求或应用。
...
- 面板上方展示订阅卡片，下方展示当前订阅的地区、节点延迟、当前节点和 JSON 结果。订阅可独立编辑、刷新或激活；地区与节点修改统一保存，页面轮询不会覆盖尚未提交的选择。订阅地址保存后会单独读取额度信息，不会因此安装配置或切换网络。
