...
面板默认固定展示“新加坡、美国、日本、香港”。其中香港默认禁用，其余三个默认启用。

韩国、印尼、大陆以及订阅里新出现的其他地区不会在 0 节点时占位置；只有配置文件里实际扫描到对应节点时才会展示出来。带旗帜的节点会自动按旗帜归类；没有可识别旗帜的节点会归入“其他地区”。这些动态地区默认启用，但香港和大陆会继续按排除规则处理。
...
- 面板在桌面宽度下会保持“运行设置 / 最近结果”左右布局；窄屏小于约 980px 时才切换为上下布局。
