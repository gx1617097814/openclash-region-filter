...
工具会自动生成并维护 OpenClash 本地配置。订阅卡片的“刷新”更新该订阅；当前订阅刷新后自动过滤并应用，待机订阅只更新私有缓存。地区开关在短暂合并连续操作后自动保存，并直接使用最后一次成功下载的本地 YAML 重新过滤，不会为一次开关操作重复请求远程订阅。
...
普通定时刷新和定时测速不会因为节点变慢或超时而自动更换当前节点，避免无提示的公网 IP 跳变。仅当用户关闭当前节点所在地区，或上次选择的节点已从新订阅中消失时，才会在仍启用的节点中优先选择已有测速记录里延迟最低的节点；没有有效测速记录时按订阅原始顺序选择第一个，并在最近结果中记录原因。订阅卡片用“启用范围”同时显示已启用地区数和其中的节点数，这些节点不一定都已通过连通性测试。
...
- 面板上方展示订阅卡片，下方展示当前订阅的地区、节点延迟、当前节点和 JSON 结果。订阅可独立编辑、刷新或激活；地区开关自动保存并应用，页面轮询不会覆盖尚在保存中的选择。
