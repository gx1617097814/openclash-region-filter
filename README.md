...
切换会先在独立缓存完成下载和过滤，再应用候选配置、测试连通性并验证运行态。工具优先恢复该订阅上次手动选择且本次可连接的节点；没有历史选择时按订阅原始顺序选择第一个可连接节点，不会按最低延迟自动选线。全部保留节点都不可连接时切换失败并恢复原配置。配置没有变化时不会重载，远程订阅临时失败时也不会删除最后一次成功生成的配置。

普通定时刷新和定时测速不会因为节点变慢或超时而自动更换当前节点，避免无提示的公网 IP 跳变。仅当上次选择的节点已从新订阅中消失时，刷新才会退回到过滤结果中的第一个保留节点，并在最近结果中记录这次变化。界面中的“保留 N 个节点”只表示通过地区规则的数量，不代表这些节点均已通过连通性测试。
