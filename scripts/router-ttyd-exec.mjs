...
function usage() {
  return [
    "Usage:",
    "  node scripts/router-ttyd-exec.mjs [--env FILE] -- COMMAND",
...
function parseArgs(argv) {
  ...
  return { envPath, checkConfig, command };
}
...
function execute(config, command) {
  return new Promise((resolve, reject) => {
    ...
    socket.addEventListener("message", async (event) => {
      try {
        const chunk = stripTerminalControls(await messageText(event.data));
        if (phase === "running") {
          output += chunk;
...
async function main() {
  ...
  const result = await execute(config, args.command);
