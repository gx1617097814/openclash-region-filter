...
当前默认策略是启用新加坡、美国、日本和未知地区节点，排除香港和大陆节点。面板固定展示新加坡、美国、日本、香港，其中香港默认禁用，其余三个默认启用。韩国、印尼、大陆以及订阅里新出现的其他地区，只有在配置文件里实际扫描到节点时才展示；这样 0 节点地区不会占用面板空间。
...
- 默认只固定展示新加坡、美国、日本、香港；其他地区只有出现节点时才展示。
...
“运行设置”和“最近结果”在桌面宽度下保持左右布局，并使用相同高度；“最近结果”内容过长时只在结果框内滚动。布局断点约为 980px，只有更窄屏幕才切换为上下排列。配置文件路径和重载命令输入行已收窄，右侧空间优先留给“最近结果”。
