...
当前默认策略是启用新加坡、美国、日本和未知地区节点，排除香港和大陆节点。面板只展示订阅里实际出现的地区，0 节点地区不占用位置；地区暂时消失时不会丢失之前保存的启用或禁用状态。
...
订阅本身已经是 Clash/OpenClash YAML 时，在面板填写订阅链接并点击“保存并应用”。工具会：
...
页面只保留订阅链接、地区开关、“立即更新订阅”和“保存并应用”。系统默认每小时自动刷新；前者按已保存规则强制执行一次完整更新，后者保存修改并立即应用。生成配置、重载和验证均由后台自动完成。最近结果同时显示摘要与可滚动的 JSON 明细。订阅额度与最近结果在桌面宽度下左右等分，窄屏时切换为上下排列。
...
执行“保存并应用”后，工具会自动确认运行态中不存在被禁用地区节点。
