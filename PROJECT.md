...
页面上方使用卡片管理多个订阅，每张卡片包含名称、地址、刷新频率、最近刷新、额度、编辑、刷新和激活状态。下方只展示当前激活订阅的地区与节点；节点列表可展开查看延迟并手动选择。订阅“刷新”和待机订阅“激活”各自承担单一职责，地区开关则会自动保存并应用。最近结果保留可滚动的 JSON 明细，包含全部节点、保留/排除节点和地区汇总。
...
地区开关自动应用后，工具会确认运行态中不存在被禁用地区节点。失败时会恢复之前的订阅缓存、输出配置和地区状态。
