...
- 测速结果仅供手动选择，不触发自动换线；切换时优先恢复该订阅上次选择的可连接节点。
...
页面上方使用卡片管理多个订阅，每张卡片包含名称、地址、刷新频率、最近刷新、额度、编辑、刷新和激活状态。下方只展示当前激活订阅的地区与节点；节点列表可展开查看延迟并手动选择。订阅“刷新”、待机订阅“激活”和地区“保存并应用”各自承担单一职责。最近结果保留可滚动的 JSON 明细。
